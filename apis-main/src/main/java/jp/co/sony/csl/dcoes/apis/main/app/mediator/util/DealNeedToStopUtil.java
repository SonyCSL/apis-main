						...
						if (old != null) {
							...
							resMap.result().replaceIfPresent(dealId, old, neo, resReplaceIfPresent -> {
								if (resReplaceIfPresent.succeeded()) {
									...
									ErrorExceptionUtil.logAndFail(Error.Category.FRAMEWORK, Error.Extent.GLOBAL, Error.Level.ERROR, resReplaceIfPresent.cause(), completionHandler);
								}
							});
						} else {
							// もともとなかった場合
							...
							resMap.result().putIfAbsent(dealId, neo, TTL_MSEC, resPutIfAbsent -> {
								if (resPutIfAbsent.succeeded()) {
									...
									ErrorExceptionUtil.logAndFail(Error.Category.FRAMEWORK, Error.Extent.GLOBAL, Error.Level.ERROR, resPutIfAbsent.cause(), completionHandler);
								}
							});
						}
					} else {
						ErrorExceptionUtil.logAndFail(Error.Category.FRAMEWORK, Error.Extent.GLOBAL, Error.Level.ERROR, resGet.cause(), completionHandler);
					}
				});
			} else {
				ErrorExceptionUtil.logAndFail(Error.Category.FRAMEWORK, Error.Extent.GLOBAL, Error.Level.ERROR, resMap.cause(), completionHandler);
			}
		});
	}
			...
			if (resMap.succeeded()) {
				resMap.result().remove(dealId, resRemove -> {
					if (resRemove.succeeded()) {
						...
						ErrorExceptionUtil.logAndFail(Error.Category.FRAMEWORK, Error.Extent.GLOBAL, Error.Level.ERROR, resRemove.cause(), completionHandler);
					}
				});
			} else {
				ErrorExceptionUtil.logAndFail(Error.Category.FRAMEWORK, Error.Extent.GLOBAL, Error.Level.ERROR, resMap.cause(), completionHandler);
			...
			if (resMap.succeeded()) {
				resMap.result().entries(resEntries -> {
					if (resEntries.succeeded()) {
						...
						ErrorExceptionUtil.logAndFail(Error.Category.FRAMEWORK, Error.Extent.GLOBAL, Error.Level.ERROR, resEntries.cause(), completionHandler);
					}
				});
			} else {
				ErrorExceptionUtil.logAndFail(Error.Category.FRAMEWORK, Error.Extent.GLOBAL, Error.Level.ERROR, resMap.cause(), completionHandler);
