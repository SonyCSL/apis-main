			...
			vertx.eventBus().<JsonObject>send(ServiceAddress.User.mediatorAccepts(), values, repAccept -> {
				if (repAccept.succeeded()) {
					...
					if (ReplyFailureUtil.isRecipientFailure(repAccept)) {
						...
						ErrorUtil.reportAndFail(vertx, Error.Category.FRAMEWORK, Error.Extent.LOCAL, Error.Level.WARN, repAccept.cause(), completionHandler);
					} else {
						ErrorUtil.reportAndFail(vertx, Error.Category.FRAMEWORK, Error.Extent.LOCAL, Error.Level.ERROR, repAccept.cause(), completionHandler);
					}
				}
			});
		} else {
