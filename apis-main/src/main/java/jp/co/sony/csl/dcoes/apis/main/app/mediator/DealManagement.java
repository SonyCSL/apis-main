						...
						vertx.eventBus().<Boolean>send(ServiceAddress.GridMaster.errorTesting(), null, repGlobalErrors -> {
							if (repGlobalErrors.succeeded()) {
									...
									CompositeFuture.<Message<Boolean>, Message<Boolean>>all(requestUnitFuture, acceptUnitFuture).setHandler(ar -> {
										if (ar.succeeded()) {
											...
											if (ReplyFailureUtil.isRecipientFailure(ar.cause())) {
												...
												ErrorUtil.reportAndFail(vertx, Error.Category.FRAMEWORK, Error.Extent.LOCAL, Error.Level.ERROR, ar.cause(), req);
											}
										}
									});
								}
								...
								if (ReplyFailureUtil.isRecipientFailure(repGlobalErrors)) {
									...
									ErrorUtil.reportAndFail(vertx, Error.Category.LOGIC, Error.Extent.LOCAL, Error.Level.WARN, repGlobalErrors.cause(), req);
								} else if (ReplyFailureUtil.isTimeout(repGlobalErrors)) {
									ErrorUtil.reportAndFail(vertx, Error.Category.FRAMEWORK, Error.Extent.LOCAL, Error.Level.WARN, repGlobalErrors.cause(), req);
								} else {
									ErrorUtil.reportAndFail(vertx, Error.Category.FRAMEWORK, Error.Extent.LOCAL, Error.Level.ERROR, repGlobalErrors.cause(), req);
								}
							}
						});
					} else {
