		...
		vertx.eventBus().send(ServiceAddress.Controller.stopLocal(), null, repStopLocal -> {
			if (repStopLocal.succeeded()) {
				...
				if (ReplyFailureUtil.isRecipientFailure(repStopLocal)) {
					...
					ErrorUtil.report(vertx, Error.Category.FRAMEWORK, Error.Extent.LOCAL, Error.Level.ERROR, repStopLocal.cause());
				}
			}
		...
		private void executeWithRetry_(Handler<AsyncResult<JsonObject>> completionHandler) {
			send_(client_, uri_, r -> {
				if (r.succeeded()) {
					...
					if (0 < --retryLimit_) {
						ErrorUtil.report(vertx, Error.Category.HARDWARE, Error.Extent.LOCAL, Error.Level.WARN, r.cause());
						executeWithRetry_(completionHandler);
					} else {
						ErrorUtil.reportAndFail(vertx, Error.Category.HARDWARE, Error.Extent.LOCAL, Error.Level.ERROR, r.cause(), completionHandler);
					}
				}
			});
		}
